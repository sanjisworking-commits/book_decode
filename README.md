...
Prototype that turns an uploaded **source JSON** (a book's chapters as structured text blocks) into a source-grounded, chapter-level **Argument Spine**, available in **English** and **Hindi-English**.

```text
source JSON → chapter extraction → AI Argument Spine → English + Hindi-English → interactive book interface
```
...
Argument Spine content is generated by the AI pipeline from user-uploaded source JSON.  
...
| Source ingest | Source-JSON upload (`/upload-json`); EPUB→JSON via Docling optional/upstream |
...
| Role | Responsibility |
|------|----------------|
| AI pipeline | Generate Argument Spine from source-JSON chapters |
