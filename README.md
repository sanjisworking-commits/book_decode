...
Prototype that turns uploaded **EPUB** or **chapter/book JSON** into a source-grounded, chapter-level **Argument Spine** (adaptive two-pass discovery → synthesis), available in **English** and optionally **Hindi-English**.

```text
Source → discovery → adaptive Argument Spine → (optional Hinglish) → interactive book interface
```
...
**Adaptive two-pass extraction + Phase 7 frontend**

Default English path: Argument Discovery → Adaptive Synthesis (`schema_version` 2.0, flexible node types + `relations[]`).  
`PROTOTYPE_ONE_SHOT=true` runs whole-chapter discovery + synthesis (chunk only over budget) and soft-validates EN-only spines.

Argument Spine content is generated by the AI pipeline from user uploads.  
